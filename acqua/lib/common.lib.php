...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    /*if ($cur_page > 1) {
        $str .= '<li class="paginate_button page-item previous disabled" id="example2_previous">';
        $str .= '<a href="'.$url.'1'.$add.'" aria-controls="example2" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-chevron-left"></i></a>';
    }*/
    ...
    if ($start_page > 1) {
        $str .= '<li class="paginate_button page-item previous" id="example2_previous">';
        $str .= '<a href="' . $url . ($start_page - 1) . $add . '" aria-controls="example2" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-chevron-left"></i>';
        $str .= '</a>';
    }
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k) {
                ...
                $str .= '<a href="'.$url.$k.$add.'" aria-controls="example2" data-dt-idx="2" tabindex="0" class="page-link">'.$k.'</a>';
                $str .= '</li>';
            } else {
                $str .= '<li class="paginate_button page-item active">';
                $str .= '<a href="'.$url.$k.$add.'" aria-controls="example2" data-dt-idx="1" tabindex="0" class="page-link">'.$k.'</a>';
            }
        }
    }

    if ($total_page > $end_page) {
        $str .= '<li class="paginate_button page-item next" id="example2_next">';
        $str .= '<a href="'.$url.($end_page+1).$add.'" aria-controls="example2" data-dt-idx="7" tabindex="0" class="page-link">';
        $str .= '       <i class="fas fa-chevron-right"></i></a>';
        $str .= '</li>';
    }

    /*if ($cur_page < $total_page) {
        $str .= '<li class="paginate_button page-item next" id="example2_next">';
        $str .= '   <a href="'.$url.$total_page.$add.'" aria-controls="example2" data-dt-idx="7" tabindex="0" class="page-link">';
    ...
    if ($str) {
        $page_html = '';
        $page_html .= '<div class="dataTables_paginate paging_simple_numbers" id="example2_paginate" style="display:inline-block">';
        $page_html .= '<ul class="pagination">'.$str.'</ul></div>';
        return $page_html;
    } else {
        return "";
    }
...
function printr($var, $exit = false)
{
    if (!is_kyowon()) return;
    $dump = '';
    $dump .= '<div align="left" style="height:100%;max-height:500px;overflow-y:scroll;line-height:20px;font-family:AppleGothic,\'나눔고딕\',NanumGothic,\'맑은 고딕\',\'Malgun Gothic\';letter-spacing:1px">';
...
function viewContents($contents, $length)
{
    $contents = str_replace('&nbsp;', ' ', $contents);
    $contents = str_replace('&amp;', '&', $contents);
    $contents = strip_tags($contents);
    $contents = conv_subject($contents, $length, '...');
    return $contents;
}

function printAjaxResponse($success = 0, $message = '', $url = '', $data = array())
{
...
function cgetAll($sql, $arr = '', $order_by = '', $debug = false)
{
    if ($arr && is_array($arr)) {
        $table = $sql;
        foreach ($arr as $key => $val) {
        ...
        $sql = " SELECT * FROM {$table} WHERE {$whereSql} {$order_by}";
    }
    $result = sql_query($sql);
    if ($debug) echo $sql;
    while ($row = sql_fetch_array($result)) {
...
function getAll($table, $arr = '', $order_by = '', $debug = false)
{
    if (strpos($table, 'g5_') === FALSE && strpos($table, 'k_') === FALSE) {
        $table = $arr ? 'g5_write_' . $table : $table;
    }
    return cgetAll($table, $arr, $order_by, $debug);
}
function kgetAll($table, $arr = '', $order_by = '', $debug = false)
{
    $table = $arr ? 'k_' . $table : $table;
    return cgetAll($table, $arr, $order_by, $debug);
}
function kdel($table, $arr = '')
{
    $table = 'k_' . $table;
    return del($table, $arr);
}
function cgetCount($table, $arr = array(), $debug = false) {
    $whereSql = '';
    if ($arr) {
        foreach ($arr as $key => $val) {
            $key_arr = explode(' ', $key);
...
function getCount($bo_table, $arr = array(), $debug = false)
{
    return cgetCount('g5_write_'.$bo_table, $arr, $debug);
}
function kgetCount($bo_table, $arr = array(), $debug = false)
{
    return cgetCount('k_'.$bo_table, $arr, $debug);
}
function cadd($table, $data, $where = '', $debug = false) {
    if (count($data) > 0) {
        $data_sql = "";
        $where_sql = "";
        $i = 0;
        ...
        if ($where) {
            ...
            {
                $where_sql .= ($i == 0 ? '' : ' AND ') . " {$key} = '{$value}' ";
                $i ++;
            }
        }

        $sql = $where ? "update" : "insert";
        $sql .= "
         {$table} set
            {$data_sql}
            {$where_sql}
...
function add($name, $data, $where = '', $debug = false) {
    $table = "g5_write_{$name}";
    return cadd($table, $data, $where, $debug);
}
function kadd($name, $data, $where = '', $debug = false) {
    $table = "k_{$name}";
    if (!$where) {
...
function kdate($datetime) {
    return str_replace('-', '.', substr($datetime, 0, 10));
}
function cget($table, $arr = '', $order_by = '', $debug = false) {
    if ($arr && is_array($arr)) {
        foreach ($arr as $key => $val) {
            $key_arr = explode(' ', $key);
            if (count($key_arr) > 1) {
        ...
        if ($order_by) $sql .= " ORDER BY " . $order_by . " LIMIT 1";
        if ($debug) {
            echo $sql;
        }
        return sql_fetch($sql);
    } else {
        return sql_fetch($table);
    }
}
function get($table, $arr = '', $order_by = '', $debug = false) {
    $table = $arr ? 'g5_write_' . $table : $table;
    return cget($table, $arr, $order_by, $debug);
}
function kget($table, $arr = '', $order_by = '', $debug = false) {
    $table = $arr ? 'k_' . $table : $table;
    return cget($table, $arr, $order_by, $debug);
}
...
function kdateString($reportCount = 0) {
    $result = date('Y').'년 '.date('n').'월 '.date('j').'일 ' . getWeekName();
    if ($reportCount > 0) $result .= ' '.$reportCount.'회차';
    return $result;
}
function kgetDate() {
    return date('Y-m-d');
}
function kgetDateTime() {
    return date('Y-m-d H:i:s');
}
function kgetAdminMenuName($code) {
    global $menu;
    $menuName = '';
    if (isset($menu['menu'.substr($code, 0, 3)])) {
        foreach ($menu['menu'.substr($code, 0, 3)] as $m) {
            if ($m[0] == $code) {
                $menuName = $m[1];
                break;
            }
...
function responseData($success, $message = '', $url = '', $data = '')
{
    echo json_encode(['success' => $success, 'message' => $message, 'url' => $url, 'data' => $data]);
    exit;
}

function is_kyowon() {
    if ($_SERVER['REMOTE_ADDR'] == '112.145.143.228') {
        return 1;
    } else {
        return 0;
    }
